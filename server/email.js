...
// registration email — shows code only, no magic link button
// (magic link would call /otp/verify and create a passwordless account,
//  bypassing the password the user entered in step 1 of registration)
export async function sendRegisterVerificationEmail(email, otp) {
  return sendEmail({
    to: email,
    subject: "Verify Your StuFi Account",
...
export async function sendDailyCapAlert(email, spent, cap) {
  return sendEmail({
    to: email,
...
export async function sendCriticalBalanceAlert(email, balance, daysLeft) {
  return sendEmail({
    to: email,
