...
import { sendOtpEmail, sendRegisterVerificationEmail, sendPasswordResetCode, sendDailyCapAlert, sendCriticalBalanceAlert } from "./email.js";
...
export async function registerRoutes(httpServer, app) {
  ...
  app.post("/api/onboarding/stories", authMiddleware, adminMiddleware, async (req, res) => {
    try {
  ...
  app.put("/api/onboarding/stories/:id", authMiddleware, adminMiddleware, async (req, res) => {
    try {
      const data = updateStorySchema.parse(req.body);
      const story = await storage.updateStory(String(req.params.id), data);
  ...
  app.delete("/api/onboarding/stories/:id", authMiddleware, adminMiddleware, async (req, res) => {
    try {
  ...
  app.post("/api/auth/register/start", authLimiter, async (req, res) => {
    try {
      ...
      await sendRegisterVerificationEmail(email, otp);
  ...
  // Cycles
  app.get("/api/cycles", authMiddleware, async (req, res) => {
    const userCycles = await storage.getCycles(req.userId);
    res.json(userCycles);
  });

  app.get("/api/cycles/current", authMiddleware, async (req, res) => {
  ...
  app.get("/api/cycles/:id", authMiddleware, async (req, res) => {
